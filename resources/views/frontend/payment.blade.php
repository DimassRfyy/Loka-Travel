...
<body class="font-poppins text-black">
    <section id="content" class="max-w-[640px] w-full mx-auto bg-[#F9F2EF] min-h-screen flex flex-col gap-8 pb-[120px]">
        ...
        <form action="{{ route('book.payment.store',$packageBooking->id) }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-8">
        @csrf
        @method('PATCH')
            ...
            <div class="bg-white p-[16px_24px] rounded-[26px] flex flex-col gap-4">
              <div class="flex flex-col gap-1">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm tracking-035 leading-[22px]">{{ $packageBooking->bank->bankname }}</span>
                </div>
              </div>
              <div class="flex flex-col gap-1">
              ...
              <div class="flex flex-col gap-1">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span id="bank-number" class="text-sm tracking-035 leading-[22px]" data-value="{{ $packageBooking->bank->bankaccountnumber }}">{{ $packageBooking->bank->bankaccountnumber }}</span>
              ...
              <div class="flex flex-col gap-1">
                ...
                <div class="flex items-center justify-between">
                  <span id="total-payment" class="font-semibold text-lg tracking-[0.6px] leading-[26px]" data-value="{{ $packageBooking->totalamount }}">Rp {{ number_format($packageBooking->totalamount) }}</span>
          ...
          <div class="navigation-bar fixed bottom-0 z-50 max-w-[640px] w-full h-[85px] bg-black rounded-t-[25px] flex items-center justify-between px-6 shadow-[-6px_-8px_20px_0_#00000008]">
            <div class="flex flex-col justify-center gap-1">
              ...
              <p id="grandtotal" class="text-[#EED202] font-semibold text-lg leading-[26px] tracking-[0.6px]">Rp {{ number_format($packageBooking->totalamount) }}</p>
            </div>
