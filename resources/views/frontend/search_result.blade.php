...
<body class="font-poppins text-black">
    <section id="content" class="max-w-[640px] w-full mx-auto bg-[#F9F2EF] min-h-screen flex flex-col gap-8 pb-[120px]">
        <nav class="mt-8 px-4 w-full flex items-center justify-between">
          ...
          <p class="text-center m-auto font-semibold">Search Result</p>
          <div class="w-12"></div>
        </nav>
        
        <div class="flex flex-col gap-3 px-4">
            <p class="text-xs">Search result for: 
                @if(request('name') && request('category') && request('city'))
                    "{{ request('name') }}" in category "{{ $categories->find(request('category'))->name }}" and city "{{ $cities->find(request('city'))->name }}"
                @elseif(request('name') && request('category'))
                    "{{ request('name') }}" in category "{{ $categories->find(request('category'))->name }}"
                @elseif(request('name') && request('city'))
                    "{{ request('name') }}" in city "{{ $cities->find(request('city'))->name }}"
                @elseif(request('category') && request('city'))
                    Category "{{ $categories->find(request('category'))->name }}" in city "{{ $cities->find(request('city'))->name }}"
                @elseif(request('name'))
                    "{{ request('name') }}"
                @elseif(request('category'))
                    Category "{{ $categories->find(request('category'))->name }}"
                @elseif(request('city'))
                    City "{{ $cities->find(request('city'))->name }}"
                @else
                    All results
                @endif</p>
         ...
         <a href="{{ route('details', $tour->slug) }}" class="card">
          ...
          @empty
          <p>No results found for "{{ request('name') }}"</p>
         @endforelse 
        </div>
